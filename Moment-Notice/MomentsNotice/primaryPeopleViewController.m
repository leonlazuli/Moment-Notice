...
- (IBAction)pair:(id)sender
{
    ...
    else
    {
        ...
        if(pairedUser.userType != ASSISTANT_USER)
        {
            UIAlertView *alert = [[UIAlertView alloc]
                                  initWithTitle:@"Error"
                                  message:@"This user is not a assistant user. Please try again."
                                  delegate:nil
                                  cancelButtonTitle:@"OK"
                                  otherButtonTitles:nil];
            [alert show];
...
-(void)reloadData
{
    ...
    {
        self.pariedOrNotLable.text = @"You haven't paired an assistant, please press pair button to pair one";
