...
- (IBAction)backButton:(id)sender
{
    ...
    [self performSegueWithIdentifier:@"eventToPrimaryMenu" sender:self];
    else if(self.user.userType == ASSISTANT_USER)
    [self performSegueWithIdentifier:@"eventToAssistantMenu" sender:self];
...
-(void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if([[segue identifier] isEqualToString:@"eventToPrimaryMenu"])
    {
        primaryMainMenuViewController* VC = [segue destinationViewController];
        VC.user = self.user;
    }
    if([[segue identifier] isEqualToString:@"eventToAssistantMenu"])
    {
...
- (void)viewDidLoad
{
    ...
    //demo
   self.eventList = [[MNEventList alloc] initWithUserFetchAllFutureEvents:self.user];
